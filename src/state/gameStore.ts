...
export type ScreenName =
  ...
  | 'cyber' | 'inazuma' | 'party' | 'dragon' | 'despedida' | 'invitacion'
...
export const useGame = create<GameState>((set, get) => ({
  ...
  startRun: (config) => {
    ...
    const run = createRun({ pools: config.pools, random: config.random, randomFlags: config.randomFlags, monotype: config.monotype, difficulty: config.difficulty, gen: config.gen, starterId: config.starterId, seed, daily: config.daily, story: config.story, starterLevel: config.starterLevel, party: config.party, sonoro: config.sonoro, freeLevel: config.freeLevel, lives: config.lives })
